...
class UserHandler:
    ...
    def follow(self, id_to_follow):
        ...
        if not usr:
            return False
        if self.user_obj.follows(id_to_follow):
            return False
    ...
    def unfollow(self, id_to_unfollow):
        usr = User.query.filter_by(id=id_to_unfollow).first()
        if not usr:
            return False
        usr.followers.remove(self.user_obj)
